...
So why does an AI struggle to propose that kind of solution up front? Largely because it usually cannot see enough of the system to justify introducing a cross cutting mechanism, and because it is implicitly trying to minimise risk. Centralising effects is a platform level decision. In our case, it touches persistence, caching, invalidation, calculations, UI display, audit trails, and possibly even anti cheat systems. If a model is only shown the battle reward function, the rational move is to keep the change local and self contained.
